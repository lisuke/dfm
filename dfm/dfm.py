...
def restore(instance):
    ...
    src_path = pathlib.Path(instance['target_dir'], app_name).expanduser().resolve()
    ...
    for relative_file in instance['relative_files']:
        abs_src_path = str(pathlib.Path.joinpath(src_path, relative_file).resolve())
        abs_tgt_path = str(pathlib.Path.joinpath(tgt_path, relative_file).resolve())
        print(colored("app_name: %s start resync relative_file: %s ." % (app_name, relative_file), "yellow"))
        rsync(abs_src_path, abs_tgt_path)
        print(colored("app_name: %s end resync relative_file: %s ." % (app_name, relative_file), "yellow"))
...
def backup(instance):
    ...
    tgt_path = pathlib.Path(instance['target_dir'], app_name).expanduser().resolve()
...
def init_instances(args):
    ...
    if args.demo:
        ...
        config_file = str(pathlib.Path(args.config_file).expanduser().resolve())

        if args.init_config:
            with open(config_file, "w") as f:
                f.write(DEMOS)
                f.close()
            print(colored("Notes: Config's file init-created. path: " + args.config_file, "green"))
            ...
            print(colored("Notes: You try '-I or --init-config' create a config's file. (.dfm.json)", "green"))
        else:
            with open(config_file, "r") as f:
                ...
                try:
                    ...
                    print(colored("ConfigFormatError: You must modify this file.", "red"))
...
def init_args():
    parser = argparse.ArgumentParser(prog="dfm", 
        description=colored("dfm is a linux tools for Dot's Files Manager. (backup or restore)", "green"))
    ...
    parser.add_argument('-C', '--config-file', dest="config_file", 
        type=str, default='~/.dfm.json', help='(Default: ~/.dfm.json)')
